...
void PrintLog(LogMessage& log, std::string print_loc, std::string file_name)
{
    ...
    if (print_loc == "SCREEN")
        printf("[%s][%d][%s][%d][%s] %s", log._level.c_str(), log._pid, log._file_name.c_str(), log._line_num, log._cur_time.c_str(), log._mes_info.c_str());
    else 
    {
        int fd = open(file_name.c_str(), O_CREAT | O_WRONLY | O_APPEND, 0666);
        char buffer[SIZE];
        memset(buffer, 0, SIZE);
        snprintf(buffer, SIZE, "[%s][%d][%s][%d][%s] %s", log._level.c_str(), log._pid, log._file_name.c_str(), log._line_num, log._cur_time.c_str(), log._mes_info.c_str());
